...
def init():
    ...
    full_path = os.path.join(base_path, lib_folder, file_name)
    print(full_path)
...
def list_vga_fps():
    return [75, 60, 50, 40, 30, 25, 20, 15, 10, 8,  5,  3,  2]

def list_ega_fps():
    return [187, 150, 137, 125, 100, 90, 75, 60, 50, 40, 37, 30, 17, 15, 12, 10, 7, 5, 3, 2]
...
class Camera:
    ...
    def grab_frame(self):
        ...
        if self.mode == CameraParameter.GRAY:
            self.frame = cv2.cvtColor(self.bayer, cv2.COLOR_BayerGB2GRAY)
        if self.mode == CameraParameter.BAYER:
            # self.frame = copy.deepcopy(self.bayer)
            self.frame = self.bayer
